...
export const retrieveRunUsageSuspenseQueryKey = (run_id: RetrieveRunUsagePathParams['run_id']) =>
  [{ url: '/v1/runs/:run_id/usage', params: { run_id: run_id } }] as const

export type RetrieveRunUsageSuspenseQueryKey = ReturnType<typeof retrieveRunUsageSuspenseQueryKey>
 ...
 */
export async function retrieveRunUsageSuspense(run_id: RetrieveRunUsagePathParams['run_id'], config: Partial<RequestConfig> & { client?: typeof client } = {}) {
...
>(
  run_id: RetrieveRunUsagePathParams['run_id'],
  options: {
    query?: Partial<UseSuspenseQueryOptions<ResponseConfig<RetrieveRunUsageQueryResponse>, ResponseErrorConfig<RetrieveRunUsage422>, TData, TQueryKey>> & {
      client?: QueryClient
    }
  ...
  const queryKey = queryOptions?.queryKey ?? retrieveRunUsageSuspenseQueryKey(run_id)

  const query = useSuspenseQuery(
    {
      ...(retrieveRunUsageSuspenseQueryOptions(run_id, config) as unknown as UseSuspenseQueryOptions),
      queryKey,
      ...(queryOptions as unknown as Omit<UseSuspenseQueryOptions, 'queryKey'>),
    },
    queryClient,
  ) as UseSuspenseQueryResult<TData, ResponseErrorConfig<RetrieveRunUsage422>> & { queryKey: TQueryKey }
