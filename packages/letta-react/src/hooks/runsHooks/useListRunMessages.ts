...
export const listRunMessagesQueryKey = (run_id: ListRunMessagesPathParams['run_id'], params?: ListRunMessagesQueryParams) =>
  [{ url: '/v1/runs/:run_id/messages', params: { run_id: run_id } }, ...(params ? [params] : [])] as const

export type ListRunMessagesQueryKey = ReturnType<typeof listRunMessagesQueryKey>
...
>(
  ...
  options: {
    query?: Partial<
      QueryObserverOptions<ResponseConfig<ListRunMessagesQueryResponse>, ResponseErrorConfig<ListRunMessages422>, TData, TQueryData, TQueryKey>
    > & { client?: QueryClient }
    client?: Partial<RequestConfig> & { client?: typeof client }
  } = {},
) {
  const { query: { client: queryClient, ...queryOptions } = {}, client: config = {} } = options ?? {}
  const queryKey = queryOptions?.queryKey ?? listRunMessagesQueryKey(run_id, params)

  const query = useQuery(
    {
      ...(listRunMessagesQueryOptions(run_id, params, config) as unknown as QueryObserverOptions),
      queryKey,
      ...(queryOptions as unknown as Omit<QueryObserverOptions, 'queryKey'>),
    },
    queryClient,
  ) as UseQueryResult<TData, ResponseErrorConfig<ListRunMessages422>> & { queryKey: TQueryKey }
