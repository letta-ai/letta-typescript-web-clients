/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

import client from '@kubb/plugin-client/clients/axios'
import type { DeleteBlockMutationResponse, DeleteBlockPathParams, DeleteBlock422 } from '../../types/DeleteBlock.ts'
import type { RequestConfig, ResponseConfig, ResponseErrorConfig } from '@kubb/plugin-client/clients/axios'
import type { UseMutationOptions, QueryClient } from '@tanstack/react-query'
import { useMutation } from '@tanstack/react-query'

export const deleteBlockMutationKey = () => [{ url: '/v1/blocks/{block_id}' }] as const

export type DeleteBlockMutationKey = ReturnType<typeof deleteBlockMutationKey>

/**
 * @summary Delete Block
 * {@link /v1/blocks/:block_id}
 */
export async function deleteBlock(block_id: DeleteBlockPathParams['block_id'], config: Partial<RequestConfig> & { client?: typeof client } = {}) {
  const { client: request = client, ...requestConfig } = config

  const res = await request<DeleteBlockMutationResponse, ResponseErrorConfig<DeleteBlock422>, unknown>({
    method: 'DELETE',
    url: `/v1/blocks/${block_id}`,
    ...requestConfig,
  })
  return res
}

/**
 * @summary Delete Block
 * {@link /v1/blocks/:block_id}
 */
export function useDeleteBlock<TContext>(
  options: {
    mutation?: UseMutationOptions<
      ResponseConfig<DeleteBlockMutationResponse>,
      ResponseErrorConfig<DeleteBlock422>,
      { block_id: DeleteBlockPathParams['block_id'] },
      TContext
    > & { client?: QueryClient }
    client?: Partial<RequestConfig> & { client?: typeof client }
  } = {},
) {
  const { mutation: { client: queryClient, ...mutationOptions } = {}, client: config = {} } = options ?? {}
  const mutationKey = mutationOptions?.mutationKey ?? deleteBlockMutationKey()

  return useMutation<
    ResponseConfig<DeleteBlockMutationResponse>,
    ResponseErrorConfig<DeleteBlock422>,
    { block_id: DeleteBlockPathParams['block_id'] },
    TContext
  >(
    {
      mutationFn: async ({ block_id }) => {
        return deleteBlock(block_id, config)
      },
      mutationKey,
      ...mutationOptions,
    },
    queryClient,
  )
}