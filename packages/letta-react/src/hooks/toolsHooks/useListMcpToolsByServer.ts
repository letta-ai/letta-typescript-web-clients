...
export const listMcpToolsByServerQueryKey = (mcp_server_name: ListMcpToolsByServerPathParams['mcp_server_name']) =>
  [{ url: '/v1/tools/mcp/servers/:mcp_server_name/tools', params: { mcp_server_name: mcp_server_name } }] as const

export type ListMcpToolsByServerQueryKey = ReturnType<typeof listMcpToolsByServerQueryKey>
...
>(
  ...
  options: {
    query?: Partial<
      QueryObserverOptions<ResponseConfig<ListMcpToolsByServerQueryResponse>, ResponseErrorConfig<ListMcpToolsByServer422>, TData, TQueryData, TQueryKey>
    > & { client?: QueryClient }
    client?: Partial<RequestConfig> & { client?: typeof client }
  } = {},
) {
  const { query: { client: queryClient, ...queryOptions } = {}, client: config = {} } = options ?? {}
  const queryKey = queryOptions?.queryKey ?? listMcpToolsByServerQueryKey(mcp_server_name)

  const query = useQuery(
    {
      ...(listMcpToolsByServerQueryOptions(mcp_server_name, config) as unknown as QueryObserverOptions),
      queryKey,
      ...(queryOptions as unknown as Omit<QueryObserverOptions, 'queryKey'>),
    },
    queryClient,
  ) as UseQueryResult<TData, ResponseErrorConfig<ListMcpToolsByServer422>> & { queryKey: TQueryKey }
