...
export const listSourceFilesQueryKey = (source_id: ListSourceFilesPathParams['source_id'], params?: ListSourceFilesQueryParams) =>
  [{ url: '/v1/sources/:source_id/files', params: { source_id: source_id } }, ...(params ? [params] : [])] as const

export type ListSourceFilesQueryKey = ReturnType<typeof listSourceFilesQueryKey>
...
>(
  ...
  options: {
    query?: Partial<
      QueryObserverOptions<ResponseConfig<ListSourceFilesQueryResponse>, ResponseErrorConfig<ListSourceFiles422>, TData, TQueryData, TQueryKey>
    > & { client?: QueryClient }
    client?: Partial<RequestConfig> & { client?: typeof client }
  } = {},
) {
  const { query: { client: queryClient, ...queryOptions } = {}, client: config = {} } = options ?? {}
  const queryKey = queryOptions?.queryKey ?? listSourceFilesQueryKey(source_id, params)

  const query = useQuery(
    {
      ...(listSourceFilesQueryOptions(source_id, params, config) as unknown as QueryObserverOptions),
      queryKey,
      ...(queryOptions as unknown as Omit<QueryObserverOptions, 'queryKey'>),
    },
    queryClient,
  ) as UseQueryResult<TData, ResponseErrorConfig<ListSourceFiles422>> & { queryKey: TQueryKey }
