...
export const sendGroupMessageMutationKey = () => [{ url: '/v1/groups/{group_id}/messages' }] as const

export type SendGroupMessageMutationKey = ReturnType<typeof sendGroupMessageMutationKey>
...
export async function sendGroupMessage(
  ...
  config: Partial<RequestConfig<SendGroupMessageMutationRequest>> & { client?: typeof client } = {},
) {
  const { client: request = client, ...requestConfig } = config

  const res = await request<SendGroupMessageMutationResponse, ResponseErrorConfig<SendGroupMessage422>, SendGroupMessageMutationRequest>({
    method: 'POST',
...
export function useSendGroupMessage<TContext>(
  options: {
    mutation?: UseMutationOptions<
      ...
      { group_id: SendGroupMessagePathParams['group_id']; data: SendGroupMessageMutationRequest },
      TContext
    > & { client?: QueryClient }
    client?: Partial<RequestConfig<SendGroupMessageMutationRequest>> & { client?: typeof client }
  } = {},
) {
