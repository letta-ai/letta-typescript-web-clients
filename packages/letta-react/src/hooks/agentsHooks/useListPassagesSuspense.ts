...
export const listPassagesSuspenseQueryKey = (agent_id: ListPassagesPathParams['agent_id'], params?: ListPassagesQueryParams) =>
  [{ url: '/v1/agents/:agent_id/archival-memory', params: { agent_id: agent_id } }, ...(params ? [params] : [])] as const

export type ListPassagesSuspenseQueryKey = ReturnType<typeof listPassagesSuspenseQueryKey>
...
>(
  ...
  params?: ListPassagesQueryParams,
  options: {
    query?: Partial<UseSuspenseQueryOptions<ResponseConfig<ListPassagesQueryResponse>, ResponseErrorConfig<ListPassages422>, TData, TQueryKey>> & {
      client?: QueryClient
    }
  ...
  const queryKey = queryOptions?.queryKey ?? listPassagesSuspenseQueryKey(agent_id, params)

  const query = useSuspenseQuery(
    {
      ...(listPassagesSuspenseQueryOptions(agent_id, params, config) as unknown as UseSuspenseQueryOptions),
      queryKey,
      ...(queryOptions as unknown as Omit<UseSuspenseQueryOptions, 'queryKey'>),
    },
    queryClient,
  ) as UseSuspenseQueryResult<TData, ResponseErrorConfig<ListPassages422>> & { queryKey: TQueryKey }
